...
''')
    ...
    def generate_swift_compile(self, target: build.BuildTarget,
        ...
        if mesonlib.version_compare(swiftc.version, '>=5.9'):
            compile_args += swiftc.get_cxx_interoperability_args(self.interpreter.compilers[target.for_machine])
        ...
        if not isinstance(target, build.Executable):
            ...
            elem.add_item('ARGS', swiftc.get_header_gen_args(out_header_name) + compile_args + module_includes)
            elem.add_item('RUNDIR', '.')
            self.add_build(elem)

            out_module_name_path = PurePath(out_module_name)
            generated_header = File(True, str(out_module_name_path.parent), out_module_name_path.name)
        else:
