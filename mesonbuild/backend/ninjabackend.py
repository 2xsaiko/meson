...
''')
    ...
    def swift_generated_header_file_name(self, target: build.BuildTarget):
        return os.path.join(self.get_target_private_dir(target),
    ...
    def generate_swift_compile(self, target: build.BuildTarget,
        ...
        if target.uses_swift_parse_as_library_args(list(itertools.chain(swift_sources.values(), swift_generated_sources))):
            compile_args += swiftc.get_library_args()
        # TODO: export header even when this target doesn't use swift
        compile_args += swiftc.get_cxx_interoperability_args(target.uses_swift_cpp_interop())
        ...
        elem.add_dep(in_module_files + rel_generated)
        elem.add_item('ARGS', swiftc.get_mod_gen_args() + mod_gen_args + header_imports + abs_generated + module_includes)
        elem.add_item('RUNDIR', rundir)
        self.add_build(elem)

        # Generate a header for importing into Obj-C/C++.
        out_header_name = self.swift_generated_header_file_name(target)
        elem = NinjaBuildElement(self.all_outputs, out_header_name, rulename, [out_module_name])
        elem.add_dep(in_module_files)
        elem.add_item('ARGS', swiftc.get_header_gen_args(out_header_name) + mod_gen_args + module_includes)
        elem.add_item('RUNDIR', '.')
        self.add_build(elem)
