...
class BuildTarget(Target):
    ...
    def process_kwargs(self, kwargs):
        ...
        if not self.swift_module_name:
            self.swift_module_name = self.name.replace(' ', '')
    ...
    def uses_swift_parse_as_library_args(self, swift_sources: T.List[File]) -> bool:
        if isinstance(self, (StaticLibrary, SharedLibrary)):
            ...
            if len(swift_sources) == 1 and swift_sources[0].fname != 'main.swift':
                return True
...
def _fill_bundle_info_from_kwargs(tgt: BundleTargetBase, kwargs) -> None:
    ...
    if len(kwargs['info_plist']) > 1:
        ...
        bi.info_dict_file = _source_input_to_file(tgt, 'info_plist', kwargs['info_plist'][0])
